...
wait_for_anvil() {
    ...
    while true; do
        ...
        now=$(date +%s)
        if (( now - start_time >= timeout )); then
            echo -e "${RED}error: anvil did not start after $timeout seconds${NOCOLOR}" >&2
            exit 1
        fi
        sleep 1
    done
...
deploy_multicall3() {
    ...
    if [[ "$code" == "0x" || -z "$code" ]]; then
        echo -e "${RED}error: failed to etch Multicall3 bytecode${NOCOLOR}" >&2
        exit 1
    fi
    echo -e "${GREEN}Multicall3 etched at $MULTICALL3_ADDR${NOCOLOR}"
}
