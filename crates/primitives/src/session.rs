...
impl SessionId {
    ...
    pub fn from_r_seed<R: rand::CryptoRng + rand::RngCore>(
        ...
        let oprf_seed = if let Some(seed) = oprf_seed {
            ...
            let random_low_bits = U256::random_with(rng) & (MASK_SESSION_OPRF_SEED - uint!(1_U256));
            let seed = random_low_bits | MASK_SESSION_OPRF_SEED;
...
mod session_id_tests {
    ...
    fn test_oprf_seed(value: u64) -> FieldElement {
        let seed = U256::from(value) | MASK_SESSION_OPRF_SEED;
        FieldElement::try_from(seed).expect("test value should fit in field")
    }

    #[test]
    fn test_new_and_accessors() {
        let commitment = test_field_element(1001);
        let action = test_oprf_seed(42);
