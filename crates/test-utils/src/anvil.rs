...
// ── State bridge constants ────────────────────────────────────────────────

/// Chain ID used for World Chain in state bridge tests.
pub const WC_CHAIN_ID: u64 = 480;
...
sol!(
    ...
    concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../contracts/out/Poseidon2V1.sol/Poseidon2T2V1.json"
    )
);
...
/// Raw commitment data extracted from a `ChainCommitted` event.
///
/// This is a test-utils-local type. Consumers (e.g. relay tests) should
/// convert this into their own domain-specific commitment type.
#[derive(Debug, Clone)]
pub struct RawChainCommitment {
    pub chain_head: B256,
...
pub async fn get_latest_chain_commitment(
    ...
    Ok(RawChainCommitment {
        ...
        block_number: decoded.data.blockNumber.try_into()?,
        chain_id: decoded.data.chainId.try_into()?,
        commitment_payload: decoded.data.commitment.clone(),
        timestamp: 0,
    })
}
...
impl TestAnvil {
    ...
    pub async fn deploy_world_id_registry_with_depth(
        ...
        let poseidon_v1 = Poseidon2T2V1::deploy(provider.clone())
            .await
            .context("failed to deploy Poseidon2T2V1 library")?;
    ...
    pub async fn deploy_world_id_registry_v2_with_depth(
        ...
        let v2_impl = Self::deploy_linked_registry_impl(
            ...
            v2_json,
            None,
            Some(*full_storage_binary_imt.address()),
            *packed_account_data.address(),
        )
        .await
    ...
    async fn deploy_linked_registry_impl<P: Provider>(
        provider: P,
        impl_json: &str,
        poseidon_v1_addr: Option<Address>,
        full_storage_binary_imt_addr: Option<Address>,
        ...
        if let Some(poseidon_v1_addr) = poseidon_v1_addr {
            bytecode_str = Self::link_bytecode_hex(
                impl_json,
                &bytecode_str,
                "src/core/hash/Poseidon2V1.sol:Poseidon2T2V1",
                poseidon_v1_addr,
            )?;
        }
        if let Some(full_storage_binary_imt_addr) = full_storage_binary_imt_addr {
            bytecode_str = Self::link_bytecode_hex(
                impl_json,
                &bytecode_str,
