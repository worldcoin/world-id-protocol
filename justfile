...
wait-for-health $BASE_PORT $COUNT $SERVICE_NAME $TIMEOUT:
    #!/usr/bin/env bash
...
run-nodes:
    ...
    # anvil wallet 7
    RUST_LOG="taceo_oprf_service=trace,world_id_oprf_node=trace,warn" ./target/release/world-id-oprf-node --bind-addr 127.0.0.1:10000 --db-connection-string postgres://postgres:postgres@localhost:5440/postgres --db-schema oprf --environment dev --version-req ">=0.2.0" > logs/node0.log 2>&1 &
    pid0=$!
    echo "started node0 with PID $pid0"
    # anvil wallet 8
    RUST_LOG="taceo_oprf_service=trace,world_id_oprf_node=trace,warn" ./target/release/world-id-oprf-node --bind-addr 127.0.0.1:10001 --db-connection-string postgres://postgres:postgres@localhost:5441/postgres --db-schema oprf --environment dev --version-req ">=0.2.0" > logs/node1.log 2>&1 &
    pid1=$!
    echo "started node1 with PID $pid1"
    # anvil wallet 9
    RUST_LOG="taceo_oprf_service=trace,world_id_oprf_node=trace,warn" ./target/release/world-id-oprf-node --bind-addr 127.0.0.1:10002 --db-connection-string postgres://postgres:postgres@localhost:5442/postgres --db-schema oprf --environment dev --version-req ">=0.2.0" > logs/node2.log 2>&1  &
    pid2=$!
...
run-setup:
    ...
    just wait-for-health 20000 3 "key-gen" 60
    ...
    just wait-for-health 10000 3 "world-id-oprf-node" 60
    read -p "press enter to stop setup"

    # cleanup
    echo "stopping OPRF nodes"
    killall world-id-oprf-node
