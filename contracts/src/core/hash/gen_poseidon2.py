...
INT = [
    ...
    "179474cceca5ff676c6bec3cef54296354391a8935ff71d6ef5aeaad7ca932f1",
...
def generate():
    ...
    l += ["pragma solidity ^0.8.13;"]
    l += ["library Poseidon2T2 {"]
    ...
    l += ["    function compress(uint256[2] memory inputs) internal pure returns (uint256) {"]
    ...
    l += ["        unchecked { uint256 s = l + r; l += s; r += s; }"]               # external_matrix_multiplication

    # first 4 external rounds
    for i in range(4):
        c0, c1 = EXT[i]
        l += [f"       unchecked {{ l += 0x{c0}; r += 0x{c1}; }}"]                  # add_external_round_constants
        l += ["        l = single_box(l); r = single_box(r);"]                      # external_s_box
        l += ["        unchecked { uint256 s = l + r; l += s; r += s; }"]           # external_matrix_multiplication

    # internal rounds
    for c in INT:
        l += [f"       unchecked {{ l += 0x{c}; }}"]                                # add_internal_round_constant
        l += ["        l = single_box(l); r = r % PRIME;"]                          # internal_s_box
        l += ["        unchecked { uint256 s = l + r; l += s; r = r + r + s; }"]    # internal_matrix_multiplication
