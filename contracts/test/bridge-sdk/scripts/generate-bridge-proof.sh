...
#
# Starts an ephemeral anvil instance, plants _validChainHeads[chainHead]=true at the
# given bridge address, mines a block, and returns ABI-encoded MPT proofs.
#
...
# _validChainHeads mapping base slot in WorldIdStateBridge
MAPPING_BASE_SLOT=9
...
# ── Plant storage ────────────────────────────────────────────────
SLOT=$(cast keccak "$(cast abi-encode 'f(bytes32,uint256)' "$CHAIN_HEAD" "$MAPPING_BASE_SLOT")")

cast rpc anvil_setCode "$BRIDGE_ADDR" "0x00" --rpc-url "$RPC" > /dev/null
cast rpc anvil_setStorageAt "$BRIDGE_ADDR" "$SLOT" \
...
# ── Fetch proofs + block ─────────────────────────────────────────
cast rpc eth_getProof "$BRIDGE_ADDR" "[\"$SLOT\"]" "$BLOCK_HEX" --rpc-url "$RPC" > "$PROOF_TMP"
