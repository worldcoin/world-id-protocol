...
/// @dev Concrete destination bridge that uses the base commitChained with real proofs.
contract DestinationBridge is WorldIdStateBridge {
    constructor(IL1Block oracle, address l1Bridge) WorldIdStateBridge(oracle, l1Bridge, 1 hours, 30, 0) {}
}
...
contract WorldChainStateBridgeTest is Test {
    ...
    function test_commitChained_multipleCommits_realL1MptProof() public {
        ...
        (BabyJubJub.Affine memory pk,) = dest._issuerSchemaIdToPubkeyAndProofId(issuerId);
        ...
        returns (bytes memory mptProof, bytes32 blockHash)
    {
        ...
        args[0] = "bash";
        args[1] = "test/bridge-sdk/scripts/generate-bridge-proof.sh";
        args[2] = "l1-to-dest";
        args[3] = rpc;
    ...
    function test_proveChainedCommitment() public {
        ...
        (bytes memory mptProof, bytes32 rootClaim) = _generateL1Proof(wcRpc, expectedHead);
        ...
        // 4. Deploy L1StateBridge
        L1StateBridge l1Bridge =
            new L1StateBridge(IDisputeGameFactory(address(factory)), PLANTED_WC_BRIDGE, 1 hours, 30, 0);

        // 5. Build CommitmentWithProof and call proveChainedCommitment
        ProofsLib.Commitment[] memory commits = new ProofsLib.Commitment[](1);
        commits[0] = ProofsLib.Commitment({blockHash: commitBlockHash, data: commitData});

        l1Bridge.proveChainedCommitment(0, ProofsLib.CommitmentWithProof({mptProof: mptProof, commits: commits}));

        // 6. Assert state was applied
    ...
    function test_proveChainedCommitment_wrongRootClaim_reverts() public {
        bytes32 commitBlockHash = bytes32(uint256(0xF00D));
        bytes memory commitData = abi.encodeWithSelector(UPDATE_ROOT_SEL, uint256(1), uint256(2), bytes32(uint256(3)));
        bytes32 expectedHead = keccak256(abi.encodePacked(bytes32(0), commitBlockHash, commitData));

        string memory wcRpc = vm.envString("WORLDCHAIN_PROVIDER");
        (bytes memory mptProof,) = _generateL1Proof(wcRpc, expectedHead);
