...
- Any function that reads from or modifies state (i.e. is not marked `pure`) must be annotated with the `onlyProxy` modifier. This ensures that it can only be called when it has access to the data in the proxy, otherwise results are likely to be nonsensical.
