...
use common::{TestGateway, spawn_test_gateway_with_local_anvil, wait_for_finalized};

const BATCH_REEVALUATION_MS: u64 = 100;

#[derive(Debug, Parser)]
#[command(about = "Load benchmark for Gateway account creation")]
struct Args {
...
async fn main() {
    ...
    let gateway = spawn_test_gateway_with_local_anvil(Some(BATCH_REEVALUATION_MS)).await;
