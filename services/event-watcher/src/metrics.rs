...
pub fn increment_rpc_error(name: &str, method: &str) {
    ::metrics::counter!(METRICS_RPC_ERRORS, "name" => name.to_owned(), "method" => method.to_owned())
        .increment(1);
}

pub fn increment_decode_error(name: &str, event_name: &str) {
    ::metrics::counter!(METRICS_DECODE_ERRORS, "name" => name.to_owned(), "event_name" => event_name.to_owned())
        .increment(1);
}

pub fn increment_reconnect(name: &str, reason: &str) {
    ::metrics::counter!(METRICS_RECONNECTS, "name" => name.to_owned(), "reason" => reason.to_owned())
        .increment(1);
}
