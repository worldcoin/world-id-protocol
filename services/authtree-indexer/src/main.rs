...
#[derive(Debug, Clone)]
struct DecodedAccountCreated {
    account_index_hex: String,
    recovery_address_bytes: Vec<u8>,
    authenticator_addresses_hex: Vec<String>,
    offchain_signer_commitment_hex: String,
}
...
async fn main() {
    ...
    let pool = make_db_pool(&cfg.db_url).await.expect("db pool");
    ...
    // After backfill, follow via WS if WS_URL is provided; otherwise poll
    if let Ok(ws_url) = std::env::var("WS_URL") {
        tracing::info!("switching to websocket live follow");
        if let Err(err) = stream_logs(&ws_url, &pool, cfg.registry_address, from).await {
            tracing::error!(?err, "websocket follow error; falling back to polling");
        }
    }
...
fn load_config() -> Config {
    ...
    Config { rpc_url, registry_address, db_url, start_block, batch_size }
}

async fn make_db_pool(db_url: &str) -> anyhow::Result<PgPool> {
...
fn decode_account_created(lg: &alloy::rpc::types::Log) -> anyhow::Result<DecodedAccountCreated> {
...
async fn insert_account(pool: &PgPool, ev: &DecodedAccountCreated) -> anyhow::Result<()> {
...
async fn stream_logs(ws_url: &str, pool: &PgPool, registry: Address, start_from: u64) -> anyhow::Result<()> {
