...
/// Create a throw-away `TreeState` backed by a temp file.
/// Depth 10 is sufficient for all tests (capacity 1024 leaves).
pub fn make_tree_state() -> TreeState {
    let path = {
        let mut p = std::env::temp_dir();
        p.push(format!("test_tree_state_{}.tmp", Uuid::new_v4()));
        p
    };
    unsafe { TreeState::new_empty(10, path).expect("failed to create tree") }
}
...
pub async fn compute_root_after_events(events: &[BlockchainEvent<RegistryEvent>]) -> U256 {
    ...
    for event in events {
        tree::apply_event_to_tree(&tree, event)
            .await
