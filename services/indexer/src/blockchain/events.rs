...
fn decode_account_recovered(lg: &alloy::rpc::types::Log) -> anyhow::Result<AccountRecoveredEvent> {
    let prim = Log::new(lg.address(), lg.topics().to_vec(), lg.data().data.clone())
        .ok_or_else(|| anyhow::anyhow!("invalid log for decoding"))?;
...
pub fn decode_registry_event(
    lg: &alloy::rpc::types::Log,
) -> anyhow::Result<BlockchainEvent<RegistryEvent>> {
    ...
    let details = match event_sig {
        WorldIdRegistry::AccountCreated::SIGNATURE_HASH => {
            RegistryEvent::AccountCreated(decode_account_created(lg)?)
        }
        WorldIdRegistry::AccountUpdated::SIGNATURE_HASH => {
            RegistryEvent::AccountUpdated(decode_account_updated(lg)?)
        }
        WorldIdRegistry::AuthenticatorInserted::SIGNATURE_HASH => {
            RegistryEvent::AuthenticatorInserted(decode_authenticator_inserted(lg)?)
        }
        WorldIdRegistry::AuthenticatorRemoved::SIGNATURE_HASH => {
            RegistryEvent::AuthenticatorRemoved(decode_authenticator_removed(lg)?)
        }
        WorldIdRegistry::AccountRecovered::SIGNATURE_HASH => {
            RegistryEvent::AccountRecovered(decode_account_recovered(lg)?)
        }
