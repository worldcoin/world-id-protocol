...
- `crates/proof/`: Proof generation/verification; ZK artifacts provided at runtime via `ZkArtifactSource`, with opt-in embed features (`embed-zkeys`, `embed-ownership-prover`/`-verifier`, `embed-passkey-prover`/`-verifier`, umbrella `embed-zk-artifacts`); nothing is embedded by default.
...
Full set of lints can be ran with: `cargo clippy --workspace --all-targets --all-features -- -D warnings`
Formatting relies on nightly options: `cargo +nightly fmt --all -- --check`
