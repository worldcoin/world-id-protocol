...
contract AttributesTest is Test {
    ...
    // ── Gateway-level compound selectors (re-declared for test access) ──
    bytes4 constant OWNED_GATEWAY_ATTRIBUTES = bytes4(keccak256("chainHead(bytes32)"));
    bytes4 constant L1_GATEWAY_ATTRIBUTES =
        bytes4(keccak256("l1ProofAttributes(uint32,bytes,bytes32[4],bytes[],bytes[])"));
    bytes4 constant ZK_GATEWAY_ATTRIBUTES = bytes4(
    ...
    function test_split_bytes32() public pure {
        ...
        (bytes4 sel, bytes memory data) = Lib.splitSelectorAndData(raw);
    ...
    function testFuzz_split_roundTrip(bytes4 sel, bytes memory payload) public pure {
        ...
        (bytes4 gotSel, bytes memory gotData) = Lib.splitSelectorAndData(raw);
    ...
    function testFuzz_split_selectorPreserved(bytes4 sel) public pure {
        ...
        (bytes4 gotSel,) = Lib.splitSelectorAndData(raw);
    ...
    function test_compoundSelectorValues() public pure {
        assertEq(OWNED_GATEWAY_ATTRIBUTES, bytes4(keccak256("chainHead(bytes32)")));
        assertEq(L1_GATEWAY_ATTRIBUTES, bytes4(keccak256("l1ProofAttributes(uint32,bytes,bytes32[4],bytes[],bytes[])")));
        assertEq(
            ZK_GATEWAY_ATTRIBUTES,
            bytes4(
                keccak256(
                    "zkProofGatewayAttributes(bytes,uint256,bytes32,bytes32,uint256,bytes32,bytes32,bytes[],bytes[])"
                )
            )
        );
    }
    ...
    function test_compoundSelectorsAreDistinct() public pure {
        bytes4[3] memory sels = [OWNED_GATEWAY_ATTRIBUTES, L1_GATEWAY_ATTRIBUTES, ZK_GATEWAY_ATTRIBUTES];
    ...
    function splitExternal(bytes memory attr) external pure returns (bytes4, bytes memory) {
        return Lib.splitSelectorAndData(attr);
    }
}
