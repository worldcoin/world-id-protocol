...
// Inclusion / non-inclusion verifier for a compressed sparse Merkle tree.
//
// Vendored from
// https://github.com/iden3/circomlib/blob/master/circuits/smt/smtverifier.circom
// with the hashes swapped for the Poseidon2 port (see smt_hash_poseidon2.circom).
//
// fnc: 0 -> verify inclusion
//      1 -> verify non-inclusion
//
// Non-inclusion has two cases, distinguished by `isOld0`:
//   isOld0 = 1: the path terminates at an empty node.
//   isOld0 = 0: the path terminates at a different leaf; the circuit then
//               enforces key != oldKey.
...
include "smt/smt_lev_ins.circom";
include "smt/smt_verifier_level.circom";
include "smt/smt_verifier_sm.circom";
