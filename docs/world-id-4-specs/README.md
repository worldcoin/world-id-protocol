...
- Verifiers MUST check the proof against the session's commitment. A proof whose `id_commitment` public signal is `0` is valid but unbound. On-chain, the dedicated `verifyWithSession()` entry point does this (it rejects `sessionId == 0`). The convenience `verify()` entry point pins the signal to `0` and rejects bound proofs, so binding is explicit in both directions.
