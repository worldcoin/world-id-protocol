...
- Verifiers MUST check the proof against the session's commitment — with the signal at `0` the proof is valid but unbound. On-chain, the dedicated `verifyWithSession()` entry point does this (it rejects `sessionId == 0`); the convenience `verify()` entry point pins the signal to `0` and rejects bound proofs, so binding is explicit in both directions.
