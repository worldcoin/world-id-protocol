...
Single-use nullifiers ensure that proofs are unlinkable. However, there are some use cases that need to make sure that it’s still the same World ID interacting with an RP. Potential use cases of that are:
...
While Session Proofs use the same zero-knowledge circuits as regular proofs, they are treated as a separate construct to call out the explicitly different handling by Authenticators when presenting them to users. Authenticators must make this type of proof explicit to users, as there is a re-used identifier.

Session Proofs work in the following manner:

- Each regular Proof exposes a commitment to the World ID account and action (called a `sessionId`).
- RPs can then store this commitment next to other account information they have, and provide this commitment to users in order to generate a Session Proof. Importantly, in order for users to prove they have the same account, the RP must provide this commitment.
- The commitment is generated as outlined below where `r` is a uniformly distributed number generated by the Authenticator.
...
a->>a: derive random blinding factor, $$r$$=KDF(rpId || action)
a->>a: store r
end 
alt [FUTURE - Derive blinding factor remotely]
a->>o: $$r$$=OPRF(rpId || action || Domain Separator)
end 
