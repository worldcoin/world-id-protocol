...
1. **Signing Key**: This is an EdDSA over BabyJubJub keypair which authenticators use to sign operations off-chain related to proof generation. The signatures from this key are used in ZK circuits (e.g. Query Circuit or [WIP-103 Proof of Ownership][wip-103]). _Previously this may have been referred to as off-chain signer_.
...
Account creation (`_registerAccount`) is unaffected. `_registerAccount` rejects `address(0)`, so every new account starts with at least one Admin Authenticator. As a consequence, an account can never be created with only Proving Authenticators, even though that is a legal end-state reachable via `insertAuthenticator` + `removeAuthenticator`.

### Manageability Invariant

An Admin Authenticator MUST NOT be removed if it is the only Admin Authenticator remaining on the account. Such a call MUST revert with `UnmanageableNotAllowed`. This guarantees that an account with one or more Proving Authenticators always retains at least one Admin Authenticator capable of managing it.
...
A general note applies to both options: practical accounts are expected to hold a small number of authenticators (the V1 default for `_maxAuthenticators` is 7 and has never been raised), so the 48-slot cap imposed by the chosen encoding is not a binding constraint. Most importantly, there is no concrete use case that warrants having standalone Management Keys. In the **future** there is a plausible evolution where Management Keys exist on Authenticator-like primitives that offer alternative mechanisms for users to hold their World ID Keys (examples: passkeys, zkLogin alternatives, etc). These constructs could provide recovery alternatives to users. This direction is not pursued in this spec because no such admin type exists. At this time there are off-protocol mechanisms (e.g. authenticator-managed backups) already address most of the same user pain points.
...
[wip-103]: https://github.com/worldcoin/world-id-protocol/blob/main/docs/WIPs/wip-103.md
