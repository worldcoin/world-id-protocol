...
1. In order for a contract, acting as an RP, to authorize requests, it MUST conform to the following interface.
  ...
  ```
   `intent` is the same public authorization used by EOA-backed requests. `oprfAction` is the exact action the node will evaluate and MAY differ from `intent.action` for session operations. The contract MUST validate that `oprfAction` is permitted by `proofType`, `sessionMode`, `actionKind`, and `action`.
...
The version of the [`ProofRequest`](https://docs.rs/world-id-primitives/latest/world_id_primitives/request/struct.ProofRequest.html) is defined in the [`RequestVersion`](https://docs.rs/world-id-primitives/latest/world_id_primitives/request/enum.RequestVersion.html) enum and is included in `RpRequestIntent`.
