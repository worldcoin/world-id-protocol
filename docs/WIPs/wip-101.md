...
The complete EIP-712 authorization and private-details commitment are specified in [RP request authorization](../world-id-4-specs/rp-request-authorization.md). A contract receives only the public intent and `detailsHash` by default. If it needs to validate private request details, `data` MAY carry those details and the salt in an RP-defined encoding; the contract SHOULD recompute and compare `detailsHash` before relying on them.
