...
1. In order for a contract, acting as an RP, to authorize legacy V1 requests, it MUST conform to the following interface.
  ```solidity
  /**
  ...
  ```
   To authorize V2 requests, a contract MUST separately advertise and implement `IWIP101V2`. The explicit interface keeps V1 byte-for-byte compatible and prevents a failed V2 authorization from being retried under V1 semantics.
  ```solidity
  ...
  ```
   `verifyRpRequestV2` MUST return `IWIP101V2.verifyRpRequestV2.selector` when the operation is authorized. `intent` is the same public authorization used by EOA-backed V2 requests. `oprfAction` is the exact action the node will evaluate and MAY differ from `intent.action` for session operations. The contract MUST validate that `oprfAction` is permitted by `proofType`, `sessionMode`, `actionKind`, and `action`.
2. If the request is not authorized, the version-specific verification method MUST revert. When requests are invalid, implementers SHOULD revert with the explicit `IWIP101.RpInvalidRequest` error. OPRF Nodes MUST handle this explicit revert and return the inner `code` to the requester (for debugging purposes).
3. Callers MUST treat any return value other than the called method's selector as an invalid request: `0x35dbc8de` for `verifyRpRequest` and `IWIP101V2.verifyRpRequestV2.selector` for `verifyRpRequestV2`.
4. The contract (implementer) MUST also adhere to [ERC-165](https://eips.ethereum.org/EIPS/eip-165) and return `true` from `supportsInterface` for each version it implements: `type(IWIP101).interfaceId`, `type(IWIP101V2).interfaceId`, or both.
...
6. The version-specific verification method will be used by OPRF Nodes to verify an incoming proof request before generating an OPRF output. OPRF Nodes MAY enforce a timeout on the resolution of this request so it is RECOMMENDED implementers don't rely on computationally expensive logic. Nodes MUST pass the exact OPRF action being evaluated: as `action` for V1 and as `oprfAction` for V2.
7. OPRF Nodes MUST use WIP-101 verification if the `signer` in the `RpRegistry` is a contract that advertises the interface for the exact request version. A V2 request MUST NOT be checked through `IWIP101`, and a V1 request MUST NOT be checked through `IWIP101V2`. OPRF Nodes MAY check whether a contract is deployed at the registered `signer` address. If there is and it supports neither interface, nodes MAY return a specific error code. OPRF Nodes are NOT REQUIRED to verify a signature via ECDSA if there is a contract deployed at the `signer` address.
...
The version of the [`ProofRequest`](https://docs.rs/world-id-primitives/latest/world_id_primitives/request/struct.ProofRequest.html) is defined in the [`RequestVersion`](https://docs.rs/world-id-primitives/latest/world_id_primitives/request/enum.RequestVersion.html) enum. V1 is passed to `verifyRpRequest`; V2 is part of the typed `RpRequestIntentV2` passed to `verifyRpRequestV2`.
...
Implementers may also authorize requests for Session Proofs or starting a session. V1 exposes only the current OPRF action and therefore relies on its prefix. V2 explicitly separates the RP's proof/session intent from the current `oprfAction`. The contract SHOULD enforce the V2 operation matrix in [RP request authorization](../world-id-4-specs/rp-request-authorization.md). For example, `Uniqueness + None` authorizes only the exact uniqueness action, while `Uniqueness + Create` additionally authorizes a fresh session-seed derivation.
...
The version-specific verification methods allow an RP to encode authorization rules in its contract for pure on-chain interactions. V2 exposes the same public intent that an EOA signs and commits to the private request body through `detailsHash`.
...
`IWIP101V2` is additive. Existing `IWIP101` contracts and V1 request bytes remain valid during the migration window, while V2 requests require explicit advertisement of the new interface.
