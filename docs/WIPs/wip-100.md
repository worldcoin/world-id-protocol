...
$\texttt{sgn0}$ is RFC 9380 §4.1 `sgn0`: the least significant bit of the canonical integer representative of its argument. $\texttt{inv0}$ is RFC 9380 §4 `inv0`: $\texttt{inv0}(0) = 0$ and $\texttt{inv0}(y) = y^{-1}$ otherwise. $\texttt{isSquare}$ is RFC 9380 §4 `is_square` and is **true for zero**. $\texttt{sqrt}$ returns an arbitrary one of the two roots; the sign is fixed explicitly in step 19 of [Elligator 2](#map-to-curve-elligator-2), so implementations MUST NOT rely on $\texttt{sqrt}$ selecting a canonical root. $\texttt{CMOV}(f, g, c)$ returns $g$ if the condition $c$ holds and $f$ otherwise.
