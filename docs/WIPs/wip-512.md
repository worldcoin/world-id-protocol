...
- `TokenSet` is defined as an abstraction over a generic mechanism to authorize a request with a server.
...
1. The server expresses its intent for a Proof of Human intent through an [x402](https://www.x402.org/) extension. The server returns a `402` HTTP Status Code, with the following 402 extension,
    ...
    - The `period` object defines the rate limit enforced by the server:
        ...
        - `period_ends_at`: The Unix epoch timestamp in seconds when the current period ends.
...
2. The server computes the `action` for which the World ID Proof is requested (per World ID specs, the RP MUST sign this as part of the `ProofRequest`). The client MUST verify the action is properly constructed with the expected form.
    ...
    action = 
      0x00 // protocol action prefix
    	...
    	+ ceil(current_ts / duration_seconds) [u32] // 4 bytes representing the current period
...
3. Upon receiving these requests, the agent will call the client with the extension object to get a valid TokenSet. The client will:
    ...
    2. Present a valid `AuthenticatorAttestation` from the Delegating Authenticator. The client obtains the attestation via a [WIP-105][wip-105] inter-authenticator communication mechanism (QR code, deeplink, or push notification). Once obtained, the client caches the attestation locally and reuses it across all RP issuance requests until expiration or until an RP's requirements are not met (see Authenticator Attestation below). If the client does not have a valid attestation, it MUST request a new one from the Delegating Authenticator before proceeding.
...
3. The client caches the attestation locally and presents it to every RP when requesting a TokenSet. The same attestation is reused across multiple RPs until it expires or fails to meet an RP's requirements.
4. When the cached attestation expires, or when an RP rejects it (e.g. due to stricter validity requirements or minimum risk statement thresholds), the client MUST request a new attestation from the Delegating Authenticator.

**Requirements:**

1. Attestations MUST NOT have a validity period exceeding **14 days**.
2. The Authenticator Attestation's `challenge` MUST be prefixed with the following bytes (remainder are random bytes):
    
    ```json
    b"agentkit"
    || 0x01 // version identifier for this spec
   	|| 0x02 // AgentKit specific context. 0x02 = WIP-106 Attestation Issuance
    ```
    
3. The Relying Party specifies in their configuration:
    1. The maximum acceptable validity period for an Authenticator Attestation. Attestations which exceed the RP's configured maximum are rejected.
    2. The minimum acceptable values for the risk statements included in the attestation.

**Why this matters:**

Presenting an Authenticator Attestation when requesting a TokenSet anchors the trust for proof generation in the Delegating Authenticator, rather than solely in the Proving Authenticator's key. This means a user must periodically re-confirm authorization (at most every 14 days), and a compromised or stolen agent key cannot be used indefinitely without the holder's ongoing consent.
...
1. A TokenSet may be usable by multiple clients from the same World ID holder. This requires a defined mechanism for securely sharing TokenSets between clients or alternative approaches such that multiple clients can obtain partial TokenSets, either of which may be a future improvement.
2. A limitation on the current definition is that it requires establishing a global rate limit period, because of how World ID Uniqueness Proofs are presented. These global periods are required to determine the context for uniqueness for a World ID Proof (the `action`). This could result in an agent obtaining a TokenSet at the end of $e_{t-1}$ and even one second after at $e_t$. The system remains safe because tokens from $e_{t-1}$ expire, but this is cumbersome user experience.
3. The x402 spec is primarily focused on payments. The extensions representation is useful to incorporate additional information, for example requiring proof of human. As x402 continues to evolve a different standard may be required for this spec.
