...
6. **Merkle inclusion proof**. The circuit MUST recompute `merkle_root` from `offchainSignerCommitment` as the leaf value at position `leaf_index` following the definitions of [WIP-100][wip-100].
...
1. Ensuring the received zero-knowledge proofs verifies correctly with the correct verifying keys.
