...
2. **Signature verification**. Compute the message of the signature as: `message = H_4(\texttt{DS\_WIP\_103}, \texttt{expected\_commitment}, \texttt{nonce}, \texttt{context})`. The circuit MUST verify `(query_r, query_s)` as an EdDSA-Poseidon2 signature by `pk` over the `message` in order to prove both knowledge of `sk` and authorization over the particular `nonce` and `context`. With $R = (\texttt{query\_r[0]}, \texttt{query\_r[1]})$ and $s = \texttt{query\_s}$, the circuit MUST constrain:
