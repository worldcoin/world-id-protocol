...
2. **Signature verification**. Compute the message of the signature as: `message = H_4(DS_WIP_103, expected_commitment, nonce, context)`. The circuit MUST verify `(query_r, query_s)` as an EdDSA-Poseidon2 signature by `pk` over the `message` in order to prove both knowledge of `sk` and authorization over the particular `nonce` and `context`. With $`R = (\mathtt{query\_r[0]}, \mathtt{query\_r[1]})`$ and $`s = \texttt{query\_s}`$, the circuit MUST constrain:
    ...
    3. **Challenge**. $`h = H_8(\texttt{DS\_EDDSA}, R_x, R_y, A_x, A_y, \mathtt{message})`$, so `h` is used as a full-field scalar without reduction.
...
9. The `nonce` and `context` MUST NOT equal to `0`.
...
1. **Provenance**. Verifies MUST generate and provide the nonce. Authenticators MUST NOT generate nonces.
2. **Single Use**. Verifies MUST enforce that a nonce may only be used one time. Verifies MUST invalidate any nonce after first use.
3. **Entropy**. Verifiers MUST generate the nonce from a uniformly random distribution using a CSPRNG over the whole field $F$.
4. **Expiration**. Nonces MUST have a defined TTL and not be valid after expiration. Verifies MAY define the mechanism to accomplish this.
