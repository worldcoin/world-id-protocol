...
For the inputs above, a `PublicKey` is an EdDSA public key represented as its coordinate points $(x,y)$ with each coordinate being a Field element. _For reference, `user_pk`, `pk_index`, `leaf_index` (named `mt_index`), `siblings` introduce the same constraints that the Nullifier Proof circuit._
...
2. **Signature verification**. Compute the message of the signature as: `message = H_4(DS_WIP_103, expected_commitment, nonce, context)`. The circuit MUST verify `(query_r, query_s)` as an EdDSA-Poseidon2 signature by `pk` over the `message` in order to prove both knowledge of `sk` and authorization over the particular `nonce` and `context`. With $`R = (\mathtt{query\_r[0]}, \mathtt{query\_r[1]})`$ and $`s = \texttt{query\_s}`$, the circuit MUST constrain:
    1. **Canonical scalar**: $s < L$ to prevent signature malleability (i.e. reduced scalar).
