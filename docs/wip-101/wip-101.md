...
5. The `verifyRpRequest` method will be used by OPRF Nodes to verify an incoming proof request before generating the required output to construct a `nullifier`. OPRF Nodes SHOULD enforce a gas limit of [X] when calling verifyRpRequest.
5. OPRF Nodes MUST use WIP-101 verification if the `signer` in the `RpRegistry` has deployed bytecode. If the contract has deployed bytecode but is not compliant with WIP-101, the OPRF Nodes MUST reject the request.
...
2. `expiresAt` is not in the past and is not too far in the future. While "too far" is relative to each RP's use case, it's unlikely that a Proof request should be valid for more than 15 minutes. Note that sub-minute precision SHOULD generally NOT be relied upon on-chain.
3. `nonce` has not been used before. This is RECOMMENDED, however OPRF Nodes will still enforce a nonce is only used once.
