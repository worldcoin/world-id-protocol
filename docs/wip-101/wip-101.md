...
6. OPRF Nodes MUST use WIP-101 verification if the `signer` in the `RpRegistry` implements [ERC-165](https://eips.ethereum.org/EIPS/eip-165) for `type(IWIP101.interfaceId)`. Otherwise the `signer` will be verified using regular ECDSA verification.
